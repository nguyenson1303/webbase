    ...
    public class PageModels
    {
        ...
        public bool Delete(int id)
        {
            using (var data = new themanorContext())
            {
                ...
                try
                {
                    var c_gen = data.Pages.Where(p => p.PageId == id).FirstOrDefault();
                    if (c_gen != null)
                    {
                        data.Pages.Remove(c_gen);
                        data.SaveChanges();
                        rt = true;
                    }
                    else
        ...
        public int Edit(Pages page)
        {
            using (var data = new themanorContext())
            {
                ...
                try
                {
                    var c_gen = data.Pages.Where(p => p.PageId == page.PageId).FirstOrDefault();
        ...
        public List<Pages> GetAllPageStatic(string lang)
        {
            using (var data = new themanorContext())
            {
                try
                {
                    var c_gen = data.Pages.Where(p => p.Show == true && (p.Lang.Trim().ToLower() == lang.Trim().ToLower())).OrderBy(p => p.OrderDisplay).ToList();
                    return c_gen;
                }
                catch (Exception)
                {
        ...
        public List<Pages> GetAllPageStaticAdmin(string lang, string search, int page_index, int page_size, string order_by, string order_type, out int total)
        {
            using (var data = new themanorContext())
            {
                try
                {
                    IQueryable<Pages> c_gen = data.Pages.Where(p => p.Show == true && p.Lang.Trim().ToLower() == lang.Trim().ToLower()).AsQueryable<Pages>();
                    if (!string.IsNullOrEmpty(search))
                    {
                        c_gen = c_gen.Where(p => p.PageName.Contains(search) || p.Link.Contains(search)).AsQueryable<Pages>();
                    }
                    ...
                    {
                        Type sortByPropType = typeof(Pages).GetProperty(order_by).PropertyType;
                        ////calling the extension method using reflection
                        c_gen = typeof(MyExtensions).GetMethod("CustomSort").MakeGenericMethod(new Type[] { typeof(Pages), sortByPropType })
                                .Invoke(c_gen, new object[] { c_gen, order_by, order_type }) as IQueryable<Pages>;
                    }
                    else
                    {
        ...
        public int GetMaxOrderDisplay()
        {
            using (var data = new themanorContext())
            {
                try
                {
                    var c_gen = data.Pages.OrderByDescending(p => p.OrderDisplay).Take(1).FirstOrDefault();
                    if (c_gen != null)
                    {
        ...
        public List<Pages> List(string lang)
        {
            using (var data = new themanorContext())
            {
                try
                {
                    var c_gen = data.Pages.Where(p => p.Lang.Trim().ToLower() == lang.Trim().ToLower()).OrderBy(p => p.OrderDisplay).ToList();
                    return c_gen;
                }
                catch (Exception)
                {
