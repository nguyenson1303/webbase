namespace ApiBase.Models.BusinessAccess
{
    ...
    public class UserModels
    {
        ...
        public List<AdminListUserPage> AdminGetAllPageFullTree(string type, string lang, string search, int parentId, int pageIndex, int pageSize, out int total)
        {
            using (var data = new themanorContext())
            {
                try
                {
                    ...
                    if (parentId != -1)
                    {
                        c_gen = (from p in data.UserPage
                                 where p.ParentId == (int)parentId
                                 select p).OrderBy(x => x.OrderDisplay).AsQueryable<UserPage>();
                    }
                    else
                    {
                        c_gen = (from p in data.UserPage
                    ...
                    return recursivePageList.Skip((pageIndex - 1) * pageSize).Take(pageSize).ToList();
                }
                catch (Exception)
                {
                    total = 0;
                ...
                if (list != null)
                {
                    foreach (var item in list)
                    {
                        ...
                        var listChild = (from p in data.UserPage
                                         ...
                                         select p).OrderBy(x => x.OrderDisplay).ToList<UserPage>();
