namespace ApiBase.Models
{
    ...
    public class CatalogModels
    {
        ...
        public List<C_Catalog> GetAllCatalogByParentID(int parent, string type, string lang, string search, int page_index, int page_size, string order_by, string order_type, out int total)
        {
            using (var data = new Entities())
            {
                try
                {
                    ...
                    return c_gen.Skip(page_index * page_size).Take(page_size).ToList<C_Catalog>(); 
                }
                catch (Exception)
                {
                    total = 0;
