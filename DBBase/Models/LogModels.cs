namespace DBBase.Models
{
    ...
    public class LogModels
    {
        ...
        public List<Log> GetAllLogByAdmin(DateTime from_date, DateTime to_date, string search, int page_index, int page_size, string order_by, string order_type, out int total)
        {
            using (var data = new Entities())
            {
                try
