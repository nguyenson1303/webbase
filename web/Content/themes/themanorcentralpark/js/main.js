    ...
    if ($('.map_image')[0]) {
        setTimeout(function () {
            $('.map_image').each(function () {
                ...
                var map = obj.find('map');

                if (img_src !== null && img_src !== undefined && img_src !== '' && image[0] && map[0]) {
                    image.attr('src', img_src);
                    image.addClass('immh');
                    var usemap = image.attr('usemap');
                    image.attr('usemap', usemap + '_' + img_id);
                    map.attr('name', usemap.replace('#', '') + '_' + img_id);
                    map.find('area').each(function () {
                        var area_map = $(this);
                        var prod_id = area_map.attr('href');
                        area_map.removeAttr('href');
                        area_map.attr('prod_id', prod_id);
                        area_map.attr('onclick', 'getProductImageList(' + prod_id + ')');
                        area_map.attr('data-maphilight', '{"strokeColor":"0000ff","strokeWidth":1,"fillColor":"ff0000","fillOpacity":0.6}');
                    });
                    $('.immh').maphilight();
                }
            });            
