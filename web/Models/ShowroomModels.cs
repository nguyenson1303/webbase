namespace Web.Models
{
    ...
    public class ShowroomModels
    {
        ...
        public PagedList<Showroom> GetListShowroom(int? districtID, int page_index, int page_site, out int total)
        {
            using (var data = new Entities())
            {
                try
                {
                    ...
                    return lstShowroom.AsQueryable().ToPagedList(page_index, page_site);
                }
                catch (Exception)
                {
                    total = 0;
